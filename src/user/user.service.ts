...
export class UserService {
  ...
  async signup(signUpDto: SignUpDto) {
    const user = await this.userRepo.findOne({ where: { email: signUpDto.email }})
    if (user) {
      return {message: "Bunday user mavjud!", accessToken: 'accessToken!!!', refreshToken: 'refreshToken!!!'}
    }
  ...
  async logout(logOutDto: LogOutDto) {
    try {
      const payload = await this.jwtService.verify(logOutDto.token, { secret: process.env.AT_SECRET })
  ...
  async getme(getMeDto: GetMeDto) {
    try {
      const payload = await this.jwtService.verify(getMeDto.token, { secret: process.env.AT_SECRET })
